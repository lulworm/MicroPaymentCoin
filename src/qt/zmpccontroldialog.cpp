...
#include "ui_zmpccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zmpcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmpcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMPC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMPC seed used to mint this zMPC is not the same as currently hold in the wallet";
            else
