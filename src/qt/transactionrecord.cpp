...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZMPCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zmpcTracker->HasMintTx(hash))) {
            //zMPC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZMPC;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by MicroPaymentCoin Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to MicroPaymentCoin Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZMPCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZMPC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZMPC || type == TransactionRecord::MNReward) {
