...
public:
    ...
    bool WriteZMPCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMPCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMPCSeed_deprecated(uint256& seed);
    bool EraseZMPCSeed();
    bool EraseZMPCSeed_deprecated();

    bool WriteZMPCCount(const uint32_t& nCount);
    bool ReadZMPCCount(uint32_t& nCount);
