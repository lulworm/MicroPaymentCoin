...
// The zMPC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMPC is a hash of the serial
...
{
    //Create an output returning the zMPC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMPCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMPC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMPC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMPCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMPC output", __func__);
...
{
    CzMPCTracker* zmpcTracker = pwallet->zmpcTracker.get();
    CMintMeta meta;
    if (!zmpcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmpcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MPC stake is the outpoint
