...
CzMPCTracker::CzMPCTracker(std::string strWalletFile)
{
...
CzMPCTracker::~CzMPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMPCTracker::Init()
{
...
bool CzMPCTracker::Archive(CMintMeta& meta)
{
...
bool CzMPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMPCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMPCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMPCTracker::GetSerialHashes()
{
...
CAmount CzMPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMPCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMPCWallet* zMPCWallet)
{
    bool iszMPCWalletInitialized = (NULL != zMPCWallet);
    CMintMeta meta;
    ...
    if (! iszMPCWalletInitialized)
        zMPCWallet = new CzMPCWallet(strWalletFile);
    meta.isSeedCorrect = zMPCWallet->CheckSeed(dMint);
    if (! iszMPCWalletInitialized)
        delete zMPCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMPCWallet* zMPCWallet = new CzMPCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMPCWallet);
        }
        delete zMPCWallet;
        LogPrint("zero", "%s: added %d dzmpc from DB\n", __func__, listDeterministicDB.size());
    }
