...
CzMPCWallet::CzMPCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmpc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMPCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMPCSeed_deprecated()) {
                    LogPrintf("%s: Updated zMPC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmpc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmpc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMPCCount(nCountLastUsed);
    else if (!walletdb.ReadZMPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMPCWallet::Lock()
{
    seedMaster = 0;
}

void CzMPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMPCWallet::LoadMintPoolFromDB()
{
...
void CzMPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMPCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmpcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmpcTracker which also adds to database
    pwalletMain->zmpcTracker->Add(dMint, true);
...
void CzMPCWallet::SeedToZMPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMPCWallet::GenerateDeterministicZMPC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMPCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMPCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
