...
/**
 * MicroPaymentCoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZmpcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzmpcseed(const UniValue& params, bool fHelp);
extern UniValue getzmpcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzmpc(const UniValue& params, bool fHelp);
extern UniValue dzmpcstate(const UniValue& params, bool fHelp);
